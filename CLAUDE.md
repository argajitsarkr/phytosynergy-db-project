...
### Open items (NOT done)
1. **Committed secrets - highest priority. Code side DONE, server side NOT DONE.**
   `docker-compose.yml` no longer contains literal secrets: `POSTGRES_PASSWORD` and
   `DJANGO_SECRET_KEY` are now `${VAR:?...}` references resolved from a gitignored
   `.env` (template committed as `.env.example`). **But the live credentials have NOT
   been rotated and are still in git history from `d92ae84` onward, with the repo
   still PUBLIC** - treat both as compromised until rotated.
   **The full procedure is in `SECRETS_ROTATION.md`. Follow it in order.**
   Two traps it exists to prevent: `.env` must be created on the server BEFORE the
   compose change is pulled (or the stack refuses to start), and the server needs its
   deploy key BEFORE the repo goes private (or `git pull` breaks). Note also that
   `POSTGRES_PASSWORD` in compose only applies on first volume init - changing an
   existing database's password needs `ALTER USER`.
2. **Going private will break the SERVER's `git pull`.** A public repo pulls anonymously;
   ...
   is unaffected - pushing already requires authentication. Covered as Stage 4 of
   `SECRETS_ROTATION.md`.
...
|------|--------|-------------|
| 2026-08-13 | - | **Security: move the committed secrets out of `docker-compose.yml`, and write the rotation + go-private runbook.** CODE SIDE ONLY - the live credentials are NOT yet rotated and the repo is still public; that is the maintainer's part, documented step by step in the new `SECRETS_ROTATION.md`. CHANGED: `docker-compose.yml` no longer holds literal secrets - `POSTGRES_PASSWORD` and `DJANGO_SECRET_KEY` are now `${VAR:?message}` references. The `:?` form is deliberate: Compose FAILS LOUDLY with that message if `.env` is missing, instead of silently substituting an empty string (which would start Postgres with a blank password and make Django raise a confusing SECRET_KEY error). ADDED `.env.example` as the committed template (`.env` itself was already gitignored and stays untracked; `.env.example` is not matched by that ignore rule, which was verified). ADDED `SECRETS_ROTATION.md`, a 6-stage runbook. THE TWO ORDERING TRAPS IT EXISTS TO PREVENT, both of which take the site or the deploy pipeline down: (1) `.env` must exist on the server BEFORE the compose change is pulled, or the stack will not start - so backup, generate, ALTER USER and write `.env` all come before `git pull`; (2) the server needs its read-only SSH deploy key and an SSH remote BEFORE the repo is flipped to private, because a public repo pulls anonymously and a private one does not - the runbook tests `git fetch` over SSH while the repo is still public, so a bad key is discovered while anonymous access is still a fallback. The laptop needs no change (pushing already required auth). THIRD TRAP documented: `POSTGRES_PASSWORD` in compose only applies when the data volume is FIRST initialised, so editing it does nothing to an existing database - the password must be changed with `ALTER USER` inside Postgres, and the two values must then agree. FOURTH TRAP: the runbook generates ALPHANUMERIC values rather than using Django's `get_random_secret_key()`, because Compose treats `$` in a `.env` value as a variable reference (silently mangling it) and `POSTGRES_PASSWORD` is embedded in `DATABASE_URL`, where `@ : / ? # %` break URL parsing. Rotating `DJANGO_SECRET_KEY` signs out every user and invalidates outstanding password-reset links - expected, and called out in the runbook. History is NOT being rewritten (maintainer's choice), so the old values stay in history from `d92ae84`; rotation is what makes them worthless, and going private removes public readability. The cloudflared credentials JSON needs no rotation - it was always mounted from the host, never committed. Verified: no literal secret remains in `docker-compose.yml`, `.env` still untracked, no em/en dashes. NOT run (nothing here can be executed on this dev machine). |
